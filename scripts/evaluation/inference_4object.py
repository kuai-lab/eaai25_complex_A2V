...
def run_inference(args, gpu_num, gpu_no, **kwargs):
    ...
    seed_list = list(range(0,100000,100))
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        for idx, indice in enumerate(range(0, len(seed_list_rank))):
            ...
            # audio 3
            sample_rate = 44100
            audio, sample_rate = librosa.load(args.audio_path3, sr=sample_rate, mono=True)
            duration = librosa.get_duration(y=audio, sr=sample_rate)
            number_audio = int(duration // 2)
            print("Audio3's number_audio:", number_audio)
            audio_inputs = { ModalityType.AUDIO: data.load_and_transform_audio_data([args.audio_path3], 'cuda', clips_per_video=number_audio),}
            audio_inputs['audio'] = audio_inputs['audio']
            ...
            output_5local = output_5local.reshape((batch_size*temporal_length, num_tokens, token_dim))
            audio_embedding = torch.cat([AudioPerceiver(output.unsqueeze(0)) for output in output_5local])
            text_emb3 = torch.cat([audio_embedding[i].unsqueeze(0).repeat_interleave(8,dim=0) for i in range(number_audio)], dim=0).to('cuda') # 1 10 1024 -> 8 10 1024 -> 40 10 1024

            # audio 4
            sample_rate = 44100
            audio, sample_rate = librosa.load(args.audio_path4, sr=sample_rate, mono=True)
            duration = librosa.get_duration(y=audio, sr=sample_rate)
            number_audio = int(duration // 2)
            print("Audio4's number_audio:", number_audio)
            audio_inputs = { ModalityType.AUDIO: data.load_and_transform_audio_data([args.audio_path4], 'cuda', clips_per_video=number_audio),}
            audio_inputs['audio'] = audio_inputs['audio']
            ...
            output_5local = output_5local.reshape((batch_size*temporal_length, num_tokens, token_dim))
            audio_embedding = torch.cat([AudioPerceiver(output.unsqueeze(0)) for output in output_5local])
            text_emb4 = torch.cat([audio_embedding[i].unsqueeze(0).repeat_interleave(8,dim=0) for i in range(number_audio)], dim=0).to('cuda') # 1 10 1024 -> 8 10 1024 -> 40 10 1024

            try:
                uc_emb[:, 1:args.num_queries+1] = text_emb1[:16]
            except:
                uc_emb[:8, 1:args.num_queries+1] = text_emb1[:8]
                uc_emb[8:16, 1:args.num_queries+1] = text_emb1[:8]

            try:
                uc_emb[:, args.num_queries+1:2*args.num_queries+1] = text_emb2[:16]
            except:
                uc_emb[:8, args.num_queries+1:2*args.num_queries+1] = text_emb2[:8]
                uc_emb[8:16, args.num_queries+1:2*args.num_queries+1] = text_emb2[:8]

            try:
                uc_emb[:, 2*args.num_queries+1:3*args.num_queries+1] = text_emb3[:16]
            except:
                uc_emb[:8, 2*args.num_queries+1:3*args.num_queries+1] = text_emb3[:8]
                uc_emb[8:16, 2*args.num_queries+1:3*args.num_queries+1] = text_emb3[:8]

            try:
                uc_emb[:, 3*args.num_queries+1:4*args.num_queries+1] = text_emb4[:16]
            except:
                uc_emb[:8, 3*args.num_queries+1:4*args.num_queries+1] = text_emb4[:8]
                uc_emb[8:16, 3*args.num_queries+1:4*args.num_queries+1] = text_emb4[:8]
